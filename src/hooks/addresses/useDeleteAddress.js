...
export function useDeleteAddress(onSuccess, onMutationStart, onMutationEnd) {
  const queryClient = useQueryClient();
  const { onClose } = useModalWindow();

  const deleteAddressMutation = useAuthMutation(deleteAddress, {
    onMutate: async () => {
    ...
    onSuccess: () => {
      ...
      onClose("delete-address")(); // Close the genres modal
