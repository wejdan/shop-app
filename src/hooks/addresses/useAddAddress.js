...
export function useAddAddress(onSuccess, onMutationStart, onMutationEnd) {
  const queryClient = useQueryClient();
  const { onClose } = useModalWindow();

  const addAddressMutation = useAuthMutation(addAddress, {
    onMutate: async () => {
    ...
    onSuccess: () => {
      ...
      onClose("add-address")(); // Close the genres modal
      toast.success("address was added successfuly", {
        duration: 5000,
