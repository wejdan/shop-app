...
export function useAddProduct(onSuccess, onMutationStart, onMutationEnd) {
  const queryClient = useQueryClient();
  const { onClose } = useModalWindow();

  const addProductMutation = useAuthMutation(createProduct, {
    onMutate: async () => {
    ...
    onSuccess: () => {
      ...
      onClose("add-product")(); // Close the genres modal
